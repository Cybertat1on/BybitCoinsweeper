...
class Tapper:
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        try:
            if settings.REF_LINK == "":
                ref_param = "referredBy=1197825376"
            else:
            ...
            sys.exit()
        # print(ref_param)
        self.ref_id = settings.REF_LINK.split('=')[2] if settings.REF_LINK != "" else "1197825376"
        if proxy:
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy):
        try:
            ...
            logger.error(f"<cyan>{self.session_name}</cyan>| Proxy: {proxy} | Error: <red>{error}</red>")
            return False
    ...
    def login(self, session: requests.Session):
        try:
            ...
            user_data = res.json()
            # print(user_data)
            logger.success(f"<cyan>{self.session_name}</cyan>| ✔️ Logged in Successfully!")
            ...
            traceback.print_exc()
            logger.error(f"<cyan>{self.session_name}</cyan>| Unknown error while trying to login: {e}")
    ...
    async def get_me(self, session):
        try:
            ...
            if res.status_code == 200:
                ...
                logger.info(f"<cyan>{self.session_name}</cyan>| Balance: <light-yellow>{user['score']}</light-yellow>")
                return True
            else:
                logger.warning(f"<cyan>{self.session_name}</cyan>| Get user info failed: <yellow>{res.status_code} | {res.json()}</yellow>")
        except:
            return False
    ...
    def refresh_token(self, session: requests.Session):
        ...
        if res.status_code == 201:
            ...
            logger.success(f"<cyan>{self.session_name}</cyan>| ✔️ Refresh token successfully")

        else:
            logger.warning(f"<cyan>{self.session_name}</cyan>| Refresh token failed: <yellow>{res.text}</yellow>")
        ...
        while True:
            try:
                if time() - jwt_token_create_time >= jwt_live_time:
                    if self.logged:
                        logger.info(f"<cyan>{self.session_name}</cyan>| Refreshing token...")
                    ...
                    while attempt_play > 0:
                        ...
                        if wl > 90:
                            try:
                                ...
                                logger.info(f"<cyan>{self.session_name}</cyan>| Wait <red>{round(sleep_)}s</red> to complete game...")
                                ...
                                # print(res.headers)
                                if res.status_code == 201:
                                    logger.info(f"<cyan>{self.session_name}</cyan>| <red>Lose game: </red><cyan>{game_id}</cyan> <red>:(</red>")
                                ...
                                logger.warning(f"<cyan>{self.session_name}</cyan>| Unknown error while trying to play game: {e}")
                        else:
                            try:
                                # print(headers)
                                head1 = {
                                ...
                                if res.status_code == 201:
                                    logger.info(
                                        f"<cyan>{self.session_name}</cyan>| ✔️ Won game : <green>{game_id}</green> | Earned <yellow>{int(lr_pl)}</yellow>")
                                    # print(res.headers)
                                    await self.get_me(session)
                                elif res.status_code == 401:
                                    self.refresh_token(session)
                                ...
                                print(res.text)
                                logger.warning(f"<cyan>{self.session_name}</cyan>| Unknown error while trying to play game: {e} - Sleep <red>20s</red>")
                                traceback.print_exc()
                ...
                traceback.print_exc()
                logger.error(f"<cyan>{self.session_name}</cyan>| Unknown error: <red>{error}</red>")
